window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** Recommended capacity for a model nothing sizes (GPT-5's input budget). */
    const RECOMMENDED_CONTEXT_WINDOW = 272000
     ...
     */
    const CONTEXT_PRESETS = [131072, 200000, 262144, 272000, 400000, 524288, 1000000, 1048576, 2000000]
    ...
    const zh = {
      ...
      'context.custom': '自定义…',
      'context.customPlaceholder': '如 272000 或 1M',
      'context.save': '保存',
      ...
      'context.hint': '探测只在点击时发生；结果需“采纳”后才写入设置。“探测报错”会向该模型发一条被拒绝的请求，不产生生成计费。',
    ...
    const en = {
      ...
      'context.customPlaceholder': 'e.g. 272000 or 1M',
      'context.save': 'Save',
      ...
      'context.hint': 'Probes run only when you click. A result changes nothing until you adopt it. “Probe refusal” sends one request the endpoint rejects before generating, so it is not billed for output.',
    ...
    class ContextProbeStore {
      constructor(api) {
        this.api = api
        this.state = { resolved: {}, listings: {}, probes: {} }
        this.listeners = new Set()
      ...
      reset() {
        ...
        this.publish({ resolved: {}, listings: {}, probes: {} })
      }
      ...
      const evidence = (label, row, status, failure) => {
        ...
        return jsx('div', {
          ...
          children: [
            jsx('span', {
              style: styles.badge,
              title: found === undefined ? undefined : String(found),
          ...
          jsx('div', {
            ...
            children: [
              ...
              jsx('button', {
                ...
                onClick: () => void probes.probeModel(provider, model.id),
