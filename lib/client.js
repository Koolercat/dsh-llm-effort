window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      'intro': '所有第三方（pi-ai）模型默认提供 low / medium / high / xhigh / max 五个 Effort。未选择时默认 max 仅适用于 catalog 会 reasoning 的模型，以及设置里白名单（forceAdaptiveThinking）的 Anthropic（anthropic-messages）模型；其余模型未选手动档时不发送 thinking。在模型旁点击“修改”，可针对单个模型取消任意 Effort；模型默认 Effort 和最后一个可用 Effort 不允许取消。',
    ...
    const en = {
      ...
      'intro': 'Every third-party (pi-ai) model gets low / medium / high / xhigh / max. Unselected default is max only for catalog reasoning models and Anthropic (anthropic-messages) models on the forceAdaptiveThinking whitelist; others omit thinking until you pick a level. Use “Modify” next to a model to remove efforts. The model default and the last remaining effort cannot be removed.',
