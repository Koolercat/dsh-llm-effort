  ...
  factory: (require) => {
    ...
    function ContextControls({ provider, model, writer, probes, t }) {
      ...
      useEffect(() => {
        ...
        // An adopted non-preset value has no matching <option>; without this the
        // select would render blank and the custom input would stay hidden.
        if (override !== undefined && !CONTEXT_PRESETS.includes(override)) setShowCustom(true)
      }, [override])
