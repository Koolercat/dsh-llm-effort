...
/**
 * Output caps are legitimately tiny (a 128-token cap is a real answer), so
 * they use their own floor instead of the window minimum.
 */
export const MIN_PROBED_MAX_TOKENS = 1
...
/**
 * A refusal of the form "inputs + max_*tokens must be <= N" names a COMBINED
 * budget (TGI's max-total-tokens): N is the input+output window, and the
 * max_*tokens token in it is an operand, not a cap of its own.
 *
 * This is matched as one whole expression and removed from the text before the
 * output patterns run. Deciding it with a lookbehind was wrong: JS lookbehind
 * is fixed-width, so "+  max_new_tokens" (two spaces) escaped it and the same
 * number was reported as both a window and an output cap.
 */
const SUM_WINDOW_PATTERN = /`?inputs?`?\s*(?:tokens)?\s*\+\s*`?max_(?:new_tokens|tokens|output_tokens|completion_tokens)`?\s*(?:tokens)?\s*(?:must\s+be\s*)?<=\s*([\d,_ ]+)/i

/**
 * Wordings that name an OUTPUT cap. Run against text with any combined-sum
 * expression already removed, so a bare match here really is an output cap.
 */
const OUTPUT_PATTERNS = [
  ...
  /max_(?:tokens|output_tokens|completion_tokens|new_tokens)`?[^\d]{0,48}?([\d,_ ]+)\s*(?:$|[.,)])/i,
]

/**
 * Explicit context-window wordings. These always win even when the same refusal
 * also names an output cap — a message can carry both numbers legitimately.
 */
const EXPLICIT_WINDOW_PATTERNS = [
...
/**
 * Ambiguous "must be <= N" wording. Only consulted when no explicit window was
 * found AND the remaining text is not naming an output / new-token cap.
 */
const AMBIGUOUS_WINDOW_PATTERNS = [
  /must\s+be\s*<=\s*([\d,_ ]+)/i,
]

/** True when a "must be <=" sentence is naming an output / completion cap. */
function ambiguousMustBeIsOutput(text) {
  return /\bmax_(?:tokens|output_tokens|completion_tokens|new_tokens)`?\s+must\s+be\s*<=/i.test(text)
    || /\b(?:max_(?:tokens|output_tokens|completion_tokens|new_tokens)|(?:maximum\s+)?(?:allowed\s+)?(?:number\s+of\s+)?(?:output|completion)\s+tokens)\b[^\n.]{0,48}must\s+be\s*<=/i.test(text)
}

function firstMatch(text, patterns, min = MIN_PROBED_CONTEXT_WINDOW) {
  for (const pattern of patterns) {
    const match = pattern.exec(text)
    if (match === null) continue
    const value = usableCapacity(match[1], { min })
 ...
 *
 * A combined "inputs + max_*tokens <= N" expression is classified first and
 * then removed, so its operand can never also be read as an output cap.
 * Explicit window wordings and output-cap wordings are otherwise independent —
 * a refusal that carries both keeps both.
 */
export function extractCapacityLimits(text) {
  if (typeof text !== 'string' || text.length === 0) return {}
  const sum = SUM_WINDOW_PATTERN.exec(text)
  const sumWindow = sum === null
    ? undefined
    : usableCapacity(sum[1], { min: MIN_PROBED_CONTEXT_WINDOW })
  // Strip the whole sum expression so its max_*tokens operand is invisible to
  // the output patterns and to the ambiguity check below.
  const rest = sum === null ? text : text.slice(0, sum.index) + ' ' + text.slice(sum.index + sum[0].length)
  // An output cap of 128 is a real answer; only WINDOWS need the 256 floor.
  const maxTokens = firstMatch(rest, OUTPUT_PATTERNS, MIN_PROBED_MAX_TOKENS)
  let contextWindow = sumWindow ?? firstMatch(rest, EXPLICIT_WINDOW_PATTERNS)
  if (contextWindow === undefined && !ambiguousMustBeIsOutput(rest)) {
    contextWindow = firstMatch(rest, AMBIGUOUS_WINDOW_PATTERNS)
  }
  return {
    ...contextWindow === undefined ? {} : { contextWindow },
    ...maxTokens === undefined ? {} : { maxTokens },
 ...
 *
 * A base URL may or may not already carry the protocol's version segment: an
 * anthropic-messages route configured as ".../anthropic" serves its catalog at
 * ".../anthropic/v1/models", so probing only "{base}/models" is a guaranteed
 * 404. When the base names no version, both spellings are tried — the
 * protocol's canonical one first.
 */
export function listingCandidates(baseUrl, api) {
  const base = String(baseUrl).replace(/\/+$/, '')
  const versioned = /\/v\d+$/.test(base)
  const candidates = []
  if (!versioned && api === 'anthropic-messages') {
    // Anthropic publishes /v1/models and nothing at the bare prefix.
    candidates.push(joinUrl(base, '/v1/models'), joinUrl(base, '/models'))
  } else {
    candidates.push(joinUrl(base, '/models'))
    if (!versioned) candidates.push(joinUrl(base, '/v1/models'))
  }
  ...
  // and never on a protocol LM Studio does not speak.
  if (versioned && api !== 'anthropic-messages') {
    candidates.push(joinUrl(base.replace(/\/v\d+$/, ''), '/api/v0/models'))
  }
  return candidates
}

/**
 * Authorization headers for a listing GET, in the scheme the route's protocol
 * actually uses. Sending a bearer token to an anthropic-messages endpoint is a
 * guaranteed 401, and spraying the key in every scheme at once would hand the
 * same secret to an endpoint that only needed one.
 */
export function listingHeaders(api, apiKey) {
  ...
  return authorized ? { authorization: `Bearer ${apiKey}` } : {}
}
...
async function readBoundedText(response, limit) {
  ...
  if (body === null || body === undefined || typeof body.getReader !== 'function') {
    try {
      ...
      // Cancellation must surface here too; reporting it as an empty body
      // would let the caller publish "unreadable" for a request the user
      // simply cancelled.
  ...
  try {
    ...
    // Cancellation must surface; a truncated body is still worth examining.
    if (isAbortError(error)) throw error
  } finally {
    try {
      await reader.cancel()
 ...
 * erase that floor.
 */
export async function probeListing({ baseUrl, api, apiKey, fetchImpl = fetch, signal, timeoutMs = DEFAULT_PROBE_TIMEOUT_MS }) {
  if (typeof baseUrl !== 'string' || baseUrl.length === 0) return { outcome: 'unsupported', models: [] }
  const auth = listingHeaders(api, apiKey)
  let lastFailure
  let successfulListing
  for (const url of listingCandidates(baseUrl, api)) {
