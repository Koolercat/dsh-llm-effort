...
/**
 * Recommended default for a model nothing sizes: 272,000 = the input budget of
 * a 400k-window model that reserves 128k for output (the GPT-5 shape). It is a
 * recommendation the settings UI offers, never an implicit default: see
 * resolveContextWindow for why the plugin refuses to guess on its own.
 */
export const RECOMMENDED_CONTEXT_WINDOW = 272000
 ...
 * the measurement, and a refused request generates no tokens.
 */
export const PROBE_MAX_TOKENS = 999999999

/** Largest error body an active probe will read before giving up. */
const MAX_PROBE_BODY_BYTES = 65536
...
export function extractCapacityLimits(text) {
  ...
  const contextWindow = firstMatch(text, WINDOW_PATTERNS)
  return {
    ...contextWindow === undefined ? {} : { contextWindow },
    ...maxTokens === undefined ? {} : { maxTokens },
  }
...
async function readBoundedText(response) {
  ...
  if (body === null || body === undefined || typeof body.getReader !== 'function') {
    try {
      return (await response.text()).slice(0, MAX_PROBE_BODY_BYTES)
    } catch {
      return ''
    }
  }
  ...
  let size = 0
  try {
    while (size < MAX_PROBE_BODY_BYTES) {
    ...
    // A truncated error body is still worth parsing.
  } finally {
    try {
      await reader.cancel()
    } catch {
...
export async function probeListing({ baseUrl, apiKey, fetchImpl = fetch, signal, timeoutMs = DEFAULT_PROBE_TIMEOUT_MS }) {
  ...
  for (const url of listingCandidates(baseUrl)) {
    ...
    // A listing that names models but sizes none is still a real answer; keep
    // it as the floor while a richer sibling endpoint is worth asking.
    lastFailure = { outcome: 'listing', models }
  }
  return lastFailure ?? { outcome: 'unreachable', models: [] }
}

/**
 * Measure one model by asking for an impossible output cap and reading the
 * refusal. Costs nothing when the endpoint validates, which is the common case;
 * an endpoint that instead accepts the request is aborted immediately and
 * reported as inconclusive rather than left generating against that cap.
 */
export async function probeByRefusal(target, { fetchImpl = fetch, signal, timeoutMs = DEFAULT_PROBE_TIMEOUT_MS } = {}) {
  ...
  if (response.ok) {
    ...
    // nothing — and must not be left generating against it.
    try {
