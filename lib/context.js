...
const OUTPUT_PATTERNS = [
  ...
  // Bare max_new_tokens is an output cap; the "inputs + max_new_tokens"
  // combined form is a total window and is excluded here.
  /(?<!\+\s)(?<!\+\s`)max_(?:tokens|output_tokens|completion_tokens|new_tokens)`?[^\d]{0,48}?([\d,_ ]+)\s*(?:$|[.,)])/i,
]
...
const EXPLICIT_WINDOW_PATTERNS = [
  ...
  // TGI: "`inputs` tokens + `max_new_tokens` must be <= 32768" names
  // max-total-tokens — the INPUT+OUTPUT window, not an output cap.
  /inputs?`?\s*(?:tokens)?\s*\+\s*`?max_new_tokens`?\s+must\s+be\s*<=\s*([\d,_ ]+)/i,
]

/**
 * Ambiguous "must be <= N" wording. Only consulted when no explicit window was
 * found AND the sentence is not naming an output / new-token cap.
 */
...
function ambiguousMustBeIsOutput(text) {
  return /(?<!\+\s)(?<!\+\s`)\bmax_(?:tokens|output_tokens|completion_tokens|new_tokens)`?\s+must\s+be\s*<=/i.test(text)
 ...
 *
 * Explicit window wordings and output-cap wordings are independent — a refusal
 * that carries both keeps both. Only the ambiguous "must be <= N" form is
 * suppressed when the sentence is clearly about an output / new-token cap.
 */
export function extractCapacityLimits(text) {
  if (typeof text !== 'string' || text.length === 0) return {}
  // An output cap of 128 is a real answer; only WINDOWS need the 256 floor.
  const maxTokens = firstMatch(text, OUTPUT_PATTERNS, MIN_PROBED_MAX_TOKENS)
  let contextWindow = firstMatch(text, EXPLICIT_WINDOW_PATTERNS)
  if (contextWindow === undefined && !ambiguousMustBeIsOutput(text)) {
    contextWindow = firstMatch(text, AMBIGUOUS_WINDOW_PATTERNS)
  }
  return {
    ...contextWindow === undefined ? {} : { contextWindow },
    ...maxTokens === undefined ? {} : { maxTokens },
