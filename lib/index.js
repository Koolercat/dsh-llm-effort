 ...
 *
 * The plugin default is max, but only for models that already reason, or
 * `anthropic-messages` models on the settings whitelist
 * (`forceAdaptiveThinking`). Catalog `reasoning: false` (claude-3-haiku, …)
 * must not materialize a default that would send thinking on every request;
 * neither must a mixed-protocol route's openai-completions models inherit max
 * just because that route is whitelisted.
 *
 ...
 *     providers:
 *       <provider route>:
 *         forceAdaptiveThinking: true   # opt-in wire flag, never inferred
 *         models:
 *           <model id>:
 *             disabledEfforts: [xhigh, max]
 *             forceAdaptiveThinking: false  # per-model override
 *
...
/**
 * Selector / request default when the route profile does not name a
 * `reasoning` level, and only for models that already reason or that the
 * settings whitelist. "Default" in the picker is not a wire value — leaving
 * `defaultEffort` unset makes LlmRuntime omit `reasoningEffort`, and pi-ai
 * then either disables thinking (anthropic-messages) or lets the gateway
 * pick its own intensity. Catalog non-reasoning models must keep that omit.
 */
export const PLUGIN_DEFAULT_EFFORT = 'max'
...
export const Config = z.object({
  ...
  providers: z.dict(z.object({
    /**
     * Opt-in: send anthropic-messages effort as `output_config.effort`
     * (adaptive thinking) for models on this route. Catalog/profile booleans
     * still win. Never inferred from `reasoning: false` — that also matches
     * catalog entries such as cloudflare-ai-gateway/claude-3-haiku.
     */
    ...
    models: z.dict(z.object({
      ...
      /** Per-model override of the route-level adaptive-thinking opt-in. */
      forceAdaptiveThinking: z.boolean(),
    })),
  })).default({}),
})
...
function routeConfigFor(config, provider) {
...
function modelConfigFor(config, provider, model) {
  const route = routeConfigFor(config, provider)
  if (route === undefined || !isRecord(route.models)) return undefined
  const entry = route.models[model]
  return isRecord(entry) ? entry : undefined
}

/**
 * Whether this plugin should fill `compat.forceAdaptiveThinking` for one
 * model. Model-level config wins over the route whitelist; both default off.
 * A catalog/profile boolean on the descriptor is a separate layer and is
 * never overwritten by this helper.
 */
export function forceAdaptiveThinkingFor(config, provider, model) {
  const entry = modelConfigFor(config, provider, model)
  if (typeof entry?.forceAdaptiveThinking === 'boolean') return entry.forceAdaptiveThinking
  const route = routeConfigFor(config, provider)
  return route?.forceAdaptiveThinking === true
}

/**
 * Whether this plugin should fill `reasoning.defaultEffort` with
 * {@link PLUGIN_DEFAULT_EFFORT}. Catalog `reasoning: false` is a real
 * "does not think" signal (claude-3-haiku). The adaptive-thinking whitelist
 * is an explicit opt-in, but only for `anthropic-messages` — mixed-protocol
 * routes (Cloudflare, Axon, …) also serve openai-completions models that
 * must not inherit max just because the route is whitelisted.
 */
export function shouldApplyPluginDefaultEffort(config, provider, model, originallyReasoning, originallyApi) {
  if (originallyReasoning === true) return true
  return originallyApi === 'anthropic-messages' && forceAdaptiveThinkingFor(config, provider, model)
}
...
/**
 * Whether this descriptor is a Grok-family model. Used only to fill
 * `supportsReasoningEffort` on openai-completions when the catalog left it
 * unset; an explicit catalog `false` (kimi, grok-4.3 on Completions, …) wins.
 */
export function isGrokFamily(model) {
  ...
  return provider === 'xai' || baseUrl.includes('api.x.ai') || /(^|[^a-z0-9])grok([^a-z0-9]|$)/.test(id)
}

/**
 * Compat flags this plugin may add for dispatch. Catalog / profile booleans
 * are never overwritten: Claude 4.5 catalog entries omit
 * `forceAdaptiveThinking` on purpose (budget_tokens), and kimi-k2.x sets
 * `supportsReasoningEffort: false` because that field is invalid on the wire.
 *
 * Fills are narrow:
 *   forceAdaptiveThinking — only when this plugin's settings whitelist the
 *     route or model (`providers.<route>.forceAdaptiveThinking`, with a
 *     per-model override). Provenance is never inferred from
 *     `reasoning: false`; that also matches catalog rows such as
 *     cloudflare-ai-gateway/claude-3-haiku.
 *   supportsReasoningEffort — Grok on openai-completions only, and only when
 *     the catalog did not already decide. xAI URL detection otherwise drops
 *     reasoning_effort.
 *
 * @returns the compat object to store, or undefined when the model had none
 *   and nothing was filled in.
 */
export function decorateDispatchCompat(model, config = {}, options = {}) {
  const hasCompat = model?.compat !== null && typeof model?.compat === 'object'
  const compat = hasCompat ? { ...model.compat } : {}
  const adaptiveAlreadySet = typeof compat.forceAdaptiveThinking === 'boolean'
  const effortAlreadySet = typeof compat.supportsReasoningEffort === 'boolean'
  ...
  if (
    ...
    && forceAdaptiveThinkingFor(config, provider, modelId)
  ) {
    compat.forceAdaptiveThinking = true
  }
  if (!effortAlreadySet && model?.api === 'openai-completions' && isGrokFamily(model)) {
    compat.supportsReasoningEffort = true
  }

  if (!hasCompat && Object.keys(compat).length === 0) return undefined
  return compat
}
...
export function decorateModel(model, config, options = {}) {
  ...
  const compat = decorateDispatchCompat(model, config, { provider, model: modelId })
 ...
 *
 * `options.originallyReasoning` / `options.originallyApi` must come from the
 * undecorated descriptor that this resolve actually used — not a second
 * `current()` read that can race a settings update.
 */
export function applyReasoningPolicy(reasoning, config, provider, model, desiredDefault, options = {}) {
  ...
  const pluginDefault = shouldApplyPluginDefaultEffort(
    config,
    provider,
    model,
    options.originallyReasoning,
    options.originallyApi,
  )
    ? PLUGIN_DEFAULT_EFFORT
    : undefined
  const defaultEffort = migrateDefaultEffort(config, provider, model, sourceDefault ?? pluginDefault)
  return {
    efforts,
...
/**
 * Original (pre-decoration) reasoning/api for the modelOf call that the
 * in-flight resolveModel is using. Captured there rather than by a second
 * `current()` in wrappedResolveModel, so a settings change between the two
 * cannot pair an old catalog `reasoning: true` with a new non-reasoning
 * descriptor.
 */
const resolveFactsStore = new AsyncLocalStorage()

function rememberOriginalModelFacts(provider, model, resolved) {
  const store = resolveFactsStore.getStore()
  if (store === undefined) return
  store.set(`${provider}\0${model}`, {
    originallyReasoning: resolved?.reasoning === true,
    originallyApi: resolved?.api,
  })
}
...
export function patchPiAiAdapter(getConfig) {
  ...
  const wrappedModelOf = function modelOfWithEfforts(snapshot, provider, model) {
    ...
    rememberOriginalModelFacts(provider, model, resolved)
  ...
  const wrappedResolveModel = async function resolveModelWithMigratedDefault(provider, model, signal) {
    const factsByModel = new Map()
    const info = await resolveFactsStore.run(factsByModel, () => originalResolveModel.call(this, provider, model, signal))
    ...
    const facts = factsByModel.get(`${provider}\0${model}`) ?? {}
    const config = activeConfig(getConfig)
    return {
      ...info,
      reasoning: applyReasoningPolicy(info.reasoning, config, provider, model, desiredDefault, {
        originallyReasoning: facts.originallyReasoning === true,
        originallyApi: facts.originallyApi,
      }),
    }
  }
