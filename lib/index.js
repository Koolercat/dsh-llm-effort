...
export function createProbeDiscovery(getAdapter, options = {}) {
  ...
  return async function discoverContextWindows(request) {
    ...
    const apiKey = await routeApiKey(adapter, provider)

    if (directive.mode === 'listing') {
      const baseUrl = request.baseURL ?? facts.find((fact) => typeof fact.baseUrl === 'string' && fact.baseUrl.length > 0)?.baseUrl
      const listing = await probeListing({ baseUrl, apiKey, ...probeOptions })
