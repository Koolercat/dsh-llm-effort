  ...
  return async function discoverContextWindows(request) {
    ...
    if (directive.mode === 'listing') {
      ...
      const apiKey = credentials.apiKey === undefined && credentials.baseUrl === configuredBaseUrl
        ...
        : credentials.apiKey
      const listing = await probeListing({ baseUrl: credentials.baseUrl, apiKey, ...probeOptions })
      const disclosed = new Map(listing.models.map((model) => [model.id, model]))
      return facts.map((fact) => {
        const found = disclosed.get(fact.id)
        const outcome = found?.contextWindow !== undefined ? 'listing'
  ...
  const wrappedPrepareCall = async function prepareCallWithEffortMigration(config, signal) {
    ...
    const wrapped = {
      ...
      stream: (options) => {
        ...
        // call returns. AsyncLocalStorage.run would drop the store at that
        // boundary, so enterWith keeps the prepare-time snapshot for the
        // remainder of this async resource — including adapter.stream's
        // modelOf reads that decide the overflow window.
        preparedConfigStore.enterWith(frozen)
        return originalPreparedStream(options)
      },
    }
